...
//using namespace std;
////함수의 매개변수
////함수에서 매개변수는 호출 시 넣어줄 값에 대한 표현입니다.
////C/C++에서는 이런 매개변수에 대한 추가 설정을 진행할 수 있습니다.
//// 
...
////함수 선언을 하고 아래에 함수를 구현할 경우 main 함수에서 호출 시 연결이 가능합니다.
////함수는 컴파일 순으로 처리됩니다.(ex) main보다 뒤에 함수가 작성된 경우라면 그 함수는 main에서 사용 불가)
...
//{
//	int temp; // 값 교환용 변수
//	va_list ap; //cstarg 헤더가 연결되어있어야 사용이 가능합니다.
//	//가변인자 리스트를 가리키는 포인터
//	va_start(ap, number); //va_list의 첫 주소를 number로 초기화합니다
//	//number만큼 작업을 진행합니다.
...
////va_start(ap, number) : 가변인자 리스트의 포인터를 첫 주소로 초기화합니다.
////va_arg(ap, int) : 해당 사이즈 만큼의 데이터를 반환하고, 포인터를 int 크기(4바이트)만큼 이동합니다.
...
//	CharacterSetting(); //디폴트가 설정되어있기에 값을 안넣어도 오류 발생이 되지 않습니다.
